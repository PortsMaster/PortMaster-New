...
love.graphics.translate(-camera.x - camera.offsetX + camera._shake_offset.x + camera._resolution_offset.x, -camera.y - camera.offsetY + camera._shake_offset.y + camera._resolution_offset.y)
end
...
if width_scale then
  if height_scale then
    ...
    camera._resolution_offset.x = math.ceil((camera.width - (smallest_scale * camera.baseWidth)) / (2 * smallest_scale))
    camera._resolution_offset.y = math.ceil((camera.height - (smallest_scale * camera.baseHeight)) / (2 * smallest_scale))

  else
    camera.setScale(width_scale, width_scale)
    camera._resolution_offset.x = math.ceil((camera.width - (width_scale * camera.baseWidth)) / (2 * width_scale))
    camera._resolution_offset.y = math.ceil((camera.height - (width_scale * camera.baseHeight)) / (2 * width_scale))
  end
elseif height_scale then
  camera.setScale(height_scale, height_scale)
  camera._resolution_offset.x = math.ceil((camera.width - (height_scale * camera.baseWidth)) / (2 * height_scale))
  camera._resolution_offset.y = math.ceil((camera.height - (height_scale * camera.baseHeight)) / (2 * height_scale))
else
...
if camera.borders then
  ...
  love.graphics.rectangle('fill', 0, 0, camera.width, camera._resolution_offset.y * camera.scaleX)
  love.graphics.rectangle('fill', 0, camera.height - camera._resolution_offset.y * camera.scaleX, camera.width, camera._resolution_offset.y * camera.scaleX)
  love.graphics.rectangle('fill', 0, camera._resolution_offset.y * camera.scaleX, camera._resolution_offset.x * camera.scaleX , camera.height - (camera._resolution_offset.y * camera.scaleX * 2))
  love.graphics.rectangle('fill', camera.width - camera._resolution_offset.x * camera.scaleX, camera._resolution_offset.y * camera.scaleX, camera._resolution_offset.x * camera.scaleX, camera.height - (camera._resolution_offset.y * camera.scaleX * 2))
