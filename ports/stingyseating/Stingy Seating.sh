...
# Variables
GAMEDIR="/$directory/ports/stingyseating"
...
# Extract file
if [ -f "./assets/Stingy Seating.exe" ]; then
  # Extract its contents in place using 7z
  pm_message "Extracting Stingy Seating.exe ..."
  ./7z x "./assets/Stingy Seating.exe" -o"./assets/" -y
...
if [ -f ./assets/game.droid ]; then
  zip -r -0 ./stingyseating.port ./assets/
  rm -Rf ./assets/
fi

# Assign configs and load the game
$GPTOKEYB "gmloadernext.aarch64" -c "stingyseating.gptk" &
