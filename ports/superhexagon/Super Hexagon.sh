...
if [ "$rocknix_mode" -eq 0 ]; then 
	$ESUDO env CRUSTY_BLOCK_INPUT=1 WRAPPED_PRELOAD="$GAMEDIR/libs.${DEVICE_ARCH}/libSDL2-2.0.so.0" WRAPPED_LIBRARY_PATH="$GAMEDIR/libs.${DEVICE_ARCH}/:$LD_LIBRARY_PATH" $weston_dir/westonwrap.sh headless noop kiosk crusty_glx_gl4es XDG_DATA_HOME=$CONFDIR BOX64_LD_LIBRARY_PATH="$GAMEDIR/libs.x64:$gamelibs" "$GAMEDIR/box64" "$gamelocation/$gamefile"
	$ESUDO $weston_dir/westonwrap.sh cleanup
else
	$ESUDO env XDG_DATA_HOME=$CONFDIR BOX64_LD_LIBRARY_PATH="$GAMEDIR/libs.x64:$gamelibs" "$GAMEDIR/box64" "$gamelocation/$gamefile"
fi
$ESUDO pkill -9 box64
