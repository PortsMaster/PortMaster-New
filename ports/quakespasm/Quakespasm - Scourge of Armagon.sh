...
if [ ! -f "$GAMEDIR/hipnotic/pak0.pak" ]; then
    ./text_viewer.${DEVICE_ARCH} -f 25 -w -t "Missing gamedata" -m "Please place your pak0.pak into the ports/quakespasm/hipnotic directory and, optionally, the music tracks as track##.ogg in ports/quakespasm/hipnotic/music.  Press 'Select' to exit this text viewer."
    exit 1
fi

if [ ! -f "$GAMEDIR/id1/pak1.pak" ] && [ "$(stat -c %s "$GAMEDIR/id1/pak0.pak")" -lt 70000000 ]; then
    ./text_viewer.${DEVICE_ARCH} -f 25 -w -t "Missing gamedata" -m "Please place your registered pak1.pak or Nightdive pak0.pak into the ports/quakespasm/id1 directory (Scourge of Armagon requires the full registered copy of Quake to run).  Press 'Select' to exit this text viewer."
    exit 1
fi

# Bug fix for Scourge of Armagon BGM not playing at startup with some source ports
