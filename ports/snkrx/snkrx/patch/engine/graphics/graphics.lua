...
function graphics.dashed_rectangle(x, y, w, h, dash_size, gap_size, color, line_width)
  local w_half = w/2
  local h_half = h/2
  graphics.dashed_line(x - w_half, y - h_half, x + w_half, y - h_half, dash_size, gap_size, color, line_width)
  graphics.dashed_line(x - w_half, y - h_half, x - w_half, y + h_half, dash_size, gap_size, color, line_width)
  graphics.dashed_line(x - w_half, y + h_half, x + w_half, y + h_half, dash_size, gap_size, color, line_width)
  graphics.dashed_line(x + w_half, y - h_half, x + w_half, y + h_half, dash_size, gap_size, color, line_width)
end
...
function graphics.triangle(x, y, w, h, color, line_width)
  local h_half = h/2
  local w_half = w/2
  local x1, y1 = x + h_half, y
  local x2, y2 = x - h_half, y - w_half
  local x3, y3 = x - h_half, y + w_half
...
function graphics.dashed_rounded_line(x1, y1, x2, y2, dash_size, gap_size, color, line_width)
  ...
  love.graphics.push()
    ...
    for i = 0, nm do
      ...
      love.graphics.rotate(math_angle(i*st, 0, i*st + dash_size, 0))
      love.graphics.translate(-i*st, -0)
      graphics.shape("rectangle", color, nil, i*st, 0, math_length((i*st + dash_size)-(i*st), 0-0), line_width/2, line_width/4, line_width/4)
      love.graphics.pop()
    end
    love.graphics.push()
    love.graphics.translate(nm*st, 0)
    love.graphics.rotate(math_angle(nm*st, 0, nm*st + dash_size, 0))
    love.graphics.translate(-nm*st, -0)
    graphics.shape("rectangle", color, nil, nm*st, 0, math_length((nm*st + dash_size)-(nm*st), 0-0), line_width/2, line_width/4, line_width/4)
    love.graphics.pop()
  love.graphics.pop()
end
    ...
    for i = 1, image_data:getWidth(), tw do
      for j = 1, image_data:getHeight(), th do
        ...
        if #yes_or_no >= (tw*th)/2 then
          out_image_data:setPixel(math_floor((i-1)/tw), math_floor((j-1)/th), 1, 1, 1, 1)
        end
      end
    end
