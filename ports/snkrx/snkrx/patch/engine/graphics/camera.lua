...
function Camera:update(dt)
  ...
  local w_half = self.w/2
  local h_half = self.h/2
  
  if self.bound then
    self.x = math_min(math_max(self.x, self.bounds_min.x + w_half), self.bounds_max.x - w_half)
    self.y = math_min(math_max(self.y, self.bounds_min.y + h_half), self.bounds_max.y - h_half)
  end
