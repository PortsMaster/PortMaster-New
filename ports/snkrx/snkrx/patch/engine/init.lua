...
function engine_run(config)
  ...
  gw, gh = config.game_width or game_width, config.game_height or game_height
  sx = state.sx or (window_width / gw)
  sy = state.sy or (window_height / gh)
  ww, wh = window_width, window_height

  state.vsync = state.vsync or 1
  state.refresh_rate = state.refresh_rate or 30
  state.frame_skip = state.frame_skip or 3
  state.msaa = state.msaa or 0
  state.sx = sx
  state.sy = sy

  love.window.setMode(gw*state.sx, gh*state.sy, { vsync = state.vsync, msaa = state.msaa, display = config.display, fullscreen = true })
  ...
  local max_frame_skip = state.frame_skip
  local max_dt_skip = fixed_dt*max_frame_skip
  local accumulator = 0
  local dt = 0
  frame, time = 0, 0
  ...
  return function()
    ...
    local lag = accumulator + dt

    if lag > max_dt_skip then
      accumulator = max_dt_skip
    else
      accumulator = lag
    end
