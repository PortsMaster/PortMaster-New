...
function EnemyCritter:die(x, y, r, n)
  ...
  for i = 1, n do HitParticle{group = main.current.effects, x = x, y = y, r = random:float(0, pi_double), color = self.color} end
...
function EnemyCritter:on_collision_enter(other, contact)
  ...
  local pi = math.pi
  local pi_half = pi/2
  if nx == 0 and ny == -1 then r = -pi_half
  elseif nx == 0 and ny == 1 then r = pi_half
  elseif nx == -1 and ny == 0 then r = pi
  else r = 0 end
