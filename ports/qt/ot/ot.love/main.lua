...
if arg[#arg] == "-debug" then require("mobdebug").start() end --(ZeroBrane IDE)
...
function love.load()
    ...
    if(save_file and #save_file > 4) then
        ...
        if(tonumber(load_volume) > 100) then load_volume = "100"; end
        if(tonumber(load_volume) < 0) then load_volume = "0"; end
...
function love.update(dt)
	...
	if(game_mode == 0) then
		...
		if(key_wait > spam_wait and key_pressed and
          (pKey == "left" or pKey == "right" or pKey == "up" or pKey == "down" or
           pKey == "w" or pKey == "a" or pKey == "s" or pKey == "d" or
           pKey == "h" or pKey == "j" or pKey == "k" or pKey == "l")) then --key mantenida, con delay aumentado hasta limite
			key_wait = 0;
...
--        love.graphics.printf("Restart", width/2-16 + 150, 6*height/8 + 128, 180, "center");
 ----------------------------------------- ADDED END
    ...
    if(#animation_list > 0) then
        anim_str = aList[1].."--"..aList[2].."--"..aList[3].."--"..aList[4].."--"..aList[5].."--"..aList[6]
    end

    local tile_anim_str = "";
    local animAList = tile_animation_list[1];
    if(#tile_animation_list > 0) then
        tile_anim_str = animAList[1].."--"..animAList[2].."--"..animAList[3].."--"..animAList[4].."--"..animAList[5].."--"..animAList[6];
    end
    ...
    if(map_file == "credits.csv") then
        ...
        local assets_credit = "https://opengameart.org/content/bird-song-1-second\nhttps://opengameart.org/content/breeze\nhttps://opengameart.org/content/20-rustles-dry-leaves\nhttps://datagoblin.itch.io/monogram\nhttps://soundcloud.com/koliao/ott"
        love.graphics.printf(assets_credit, rect_x, 5*rect_y/8, rect_x*2/(rect_y/270), "center", 0, rect_y/270);
    ...
    if(pausa) then
        ...
        if(menu_selected == 2) then
            ...
            love.graphics.setColor(245/255, 245/255, 200/255);
            love.graphics.printf(str_volume, width/2 + dMenuX + 40, dHeight*(menu_selected + 2), 10*180/8, "center");
