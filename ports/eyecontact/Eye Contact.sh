...
# Variables
GAMEDIR="/$directory/ports/eyecontact"
...
# Extract file
if [ -f "./assets/Eye Contact.exe" ]; then
  # Extract its contents in place using 7z
  pm_message "Extracting Eye Contact.exe ..."
  ./7z x "./assets/Eye Contact.exe" -o"./assets/" -y
...
if [ -f ./assets/game.droid ]; then
  zip -r -0 ./eyecontact.port ./assets/
  rm -Rf ./assets/
fi

# Assign configs and load the game
$GPTOKEYB "gmloadernext.aarch64" -c "eyecontact.gptk" &
