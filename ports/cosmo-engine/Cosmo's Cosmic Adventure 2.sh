...
if [ ! -f "$GAMEDIR/data/cosmo2.vol" ] && [ ! -f "$GAMEDIR/data/COSMO2.VOL" ]; then
    ./text_viewer.${DEVICE_ARCH} -f 18 -w -t "Instructions" --input_file "$GAMEDIR/ep2inst.txt"
fi

$GPTOKEYB "cosmo_engine" -c "$GAMEDIR/cosmo_engine.gptk" &
pm_platform_helper "$GAMEDIR/cosmo_engine"
./cosmo_engine -datadir data -savedir data -gamedir data -ep2 $ADDLPARAMS
