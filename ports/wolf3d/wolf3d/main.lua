...
local currentPath = "wolffiles"  -- Start in the main wolffiles folder
local previousPath = ""
...
function handleSubMenu(button)
    ...
    elseif button == "B" then
        if currentPath ~= "wolffiles" then  -- If not in the root folder
...
function drawRadioButtons()
    ...
    for i, label in ipairs({"EC Wolf", "LZ Wolf"}) do
    ...
    local buttonsWidth = #({"EC Wolf", "LZ Wolf"}) * (radioButtonSize + radioButtonSpacing) - radioButtonSpacing
    local buttonsCenterX = radioX + buttonsWidth / 2
    local helpTextX = buttonsCenterX - helpTextWidth / 2 + (windowWidth * 0.02)
...
function handleSelection()
    ...
    local radioOptions = { "ecwolf", "lzwolf" }
    ...
    if selectedGame == "Exit" then
        ...
        fullGamePath = currentPath .. "/" .. selectedGame .. ".wolf"
    end
