...
if [ -f "$LIBRARY_PATH" ]; then
    ...
    [[ "$CURRENT_ARCH" == "aarch64" ]] && pm_message  "$FILE_INFO" | grep -q "aarch64\|ARM aarch64"
else
...
if [ -n "$GDNLIB_FILES" ]; then
    pm_message  "Archivos .gdnlib encontrados:"
    for gdnlib in $GDNLIB_FILES; do
        pm_message  "📄 $gdnlib"
        pm_message  "Contenido relevante:"
        grep -E "(entry/|dependency/)" "$gdnlib" | head -10
    done
else
    pm_message  "⚠️  No se encontraron archivos .gdnlib"
fi
