...
- **Cleans up after each blind.** Stock Balatro leaves a particle emitter behind every time you defeat a blind; over a long run those pile up and the game slows down even with effects off. This port removes them, and also periodically reaps other spent empty particle emitters and leftover `REMOVED` objects. To keep the stock behaviour, set `BALATRO_PM_SKIP_PARTICLE_CLEANUP=1` in `Balatro.sh`.
- **CPU opts even with Performance Off.** Settled deck/discard cards skip idle updates, unused per-card shadow Moveables are dropped, pile layout is cached when still, GC stays running (stock stops it every frame), and the frame rate is capped (60 by default; 40 or 30 from display setup or Settings → Video). HUD chip/mult text reuses and releases Love2D Text glyphs so a long run does not sag under a flat `mv` count. None of that changes the desktop look. Disable with `BALATRO_PM_SKIP_CPU_OPT=1` (text cleanup: `BALATRO_PM_SKIP_TEXT_CLEANUP=1`). Performance On still owns the heavier graphics cuts (CRT, bloom, shadows, reduced motion, fewer particles, static background).
- **Spanish (Mexico) score popups.** Stock `es_419` copies a Russian leftover into joker mult popups (`+N множ.`). Nunito shows those glyphs; the pixel font hid them. This port replaces it with Spanish (Spain) wording (`+N Multi.`).
