...
- **CPU opts even with Performance Off.** Settled deck/discard cards skip idle updates, unused per-card shadow Moveables are dropped, pile layout is cached when still, GC steps harder (with a periodic full collect), and the frame rate is capped (60 by default; 40 or 30 from display setup or Settings → Video). HUD chip/mult text also releases Love2D Text objects so FPS stays at the cap on long runs. None of that changes the desktop look. Disable with `BALATRO_PM_SKIP_CPU_OPT=1` (text cleanup: `BALATRO_PM_SKIP_TEXT_CLEANUP=1`). Performance On still owns the heavier graphics cuts (CRT, bloom, shadows, reduced motion, fewer particles, static background).
