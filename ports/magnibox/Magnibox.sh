...
# /opt/system/Tools/PortMaster/xdelta3 -f -e -s "gamedata/data.win.steam" "gamedata/game.droid" "patch/magnibox.steam.patch"
# /opt/system/Tools/PortMaster/xdelta3 -f -e -s "gamedata/data.win.itch" "gamedata/game.droid" "patch/magnibox.itch.patch"

# zip -x 'log.txt' '*magnibox_savedata' '*.droid' '*.win' '*.unx' '*.dat' -r "Magnibox.zip" "Magnibox.sh" magnibox

# check if we have new engough version of PortMaster that contains xdelta3
if [ ! -f "$controlfolder/xdelta3.${DEVICE_ARCH}" ]; then
...
if [ -f "./gamedata/data.win" ]; then
    ...
    if [ "$checksum" = "ee45b1cde7b43538b73df84a29dfc6a0" ]; then # itch.io version
        $ESUDO $controlfolder/xdelta3.${DEVICE_ARCH} -d -s gamedata/data.win -f ./patch/magnibox.itch.patch gamedata/game.droid && \
        rm gamedata/data.win
    elif [ "$checksum" = "30427fca0a9946d09a06e38aa2f4b41d" ]; then # steam version
        $ESUDO $controlfolder/xdelta3.${DEVICE_ARCH} -d -s gamedata/data.win -f ./patch/magnibox.steam.patch gamedata/game.droid && \
        rm gamedata/data.win
    else
