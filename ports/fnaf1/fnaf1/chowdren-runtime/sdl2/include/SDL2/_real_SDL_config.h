...
/* #undef HAVE_IMMINTRIN_H -- was hardcoded to 1 in the vendored config, which
   force-includes x86-only immintrin.h/mmintrin.h in SDL_cpuinfo.h regardless
   of target arch. Left undefined here so the __MMX__/__SSE__-gated fallback
   branch in SDL_cpuinfo.h is used instead, which correctly no-ops on aarch64. */
