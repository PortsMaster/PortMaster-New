...
# Variables
GAMEDIR="/$directory/ports/overwrite"
...
# Extract file
if [ -f "./assets/Overwrite.exe" ]; then
  # Extract its contents in place using 7z
  pm_message "Extracting Overwrite.exe ..."
  ./7z x "./assets/Overwrite.exe" -o"./assets/" -y
...
if [ -f ./assets/game.droid ]; then
  zip -r -0 ./overwrite.port ./assets/
  rm -Rf ./assets/
fi

# Assign configs and load the game
$GPTOKEYB "gmloadernext.aarch64" -c "overwrite.gptk" &
