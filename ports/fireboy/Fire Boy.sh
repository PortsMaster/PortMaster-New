...
GAMEDIR=/$directory/ports/fireboy
godot_runtime="godot_4.3"
godot_executable="godot43.$DEVICE_ARCH"
pck_filename="FireBoyPM.pck"
gptk_filename="FireBoy.gptk"
...
if [ -f "$GAMEDIR/gamedata/Fire Boy.pck" ]; then
  $controlfolder/xdelta3 -d -s "$GAMEDIR/gamedata/Fire Boy.pck" "patch.xdelta" "$GAMEDIR/gamedata/FireBoyPM.pck"
  rm -f "$GAMEDIR/gamedata/Fire Boy.pck"
fi
