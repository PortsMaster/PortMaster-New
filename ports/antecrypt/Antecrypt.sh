...
# Variables
GAMEDIR="/$directory/ports/antecrypt"
BINARY="gamedata/antecrypt.app/Contents/MacOS/antecrypt"
...
if command -v python3 >/dev/null 2>&1 \
   && [ -f "$GAMEDIR/tools/sugar_fingerprint.py" ]; then
    for candidate in "$PATCHES_DIR"/antecrypt*.conf; do
    ...
    case "$BIN_SIZE" in
        5399168)  PATCHES="$PATCHES_DIR/antecrypt.conf" ;;
    esac
fi
if [ -z "$PATCHES" ]; then
    pm_message "Unrecognised Antecrypt binary"
    sleep 15
    exit 1
fi
