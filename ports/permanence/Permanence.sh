...
# Variables
GAMEDIR="/$directory/ports/permanence"
...
# Extract file
if [ -f "./assets/Permanence.exe" ]; then
  # Extract its contents in place using 7z
  pm_message "Extracting Permanence.exe ..."
  ./7z x "./assets/Permanence.exe" -o"./assets/" -y
...
if [ -f ./assets/game.droid ]; then
  zip -r -0 ./permanence.port ./assets/
  rm -Rf ./assets/
fi

# Assign configs and load the game
$GPTOKEYB "gmloadernext.aarch64" -c "permanence.gptk" &
