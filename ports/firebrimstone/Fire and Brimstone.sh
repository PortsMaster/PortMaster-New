...
# Variables
GAMEDIR="/$directory/ports/firebrimstone"
BINARY="gamedata/fire_brimstone.app/Contents/MacOS/fire_brimstone"
...
if command -v python3 >/dev/null 2>&1 \
   && [ -f "$GAMEDIR/tools/sugar_fingerprint.py" ]; then
    for candidate in "$PATCHES_DIR"/fire_brimstone*.conf; do
    ...
    case "$BIN_SIZE" in
         4959248)  PATCHES="$PATCHES_DIR/fire_brimstone.conf" ;;
    esac
fi
if [ -z "$PATCHES" ]; then
    pm_message "Unrecognised Fire & Brimstone — see BUILDING.md"
    sleep 15
    exit 1
fi
