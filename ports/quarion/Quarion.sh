...
GAMEDIR=/$directory/ports/quarion
godot_runtime="godot_4.2.2"
godot_executable="godot422.$DEVICE_ARCH"
pck_filename="Quarionpatched.exe"
gptk_filename="quarion.gptk"
...
# If "Quarion.exe" exists and its MD5 checksum matches the specified value, apply the xdelta3 patch
if [ -f "./Quarion.exe" ]; then
    checksum=$(md5sum "./Quarion.exe" | awk '{print $1}')
    if [ "$checksum" = "6abf166118a6357c671e61b197363bd0" ]; then
        $ESUDO $controlfolder/xdelta3 -d -s ./Quarion.exe -f ./patch/quarion.xdelta ./Quarionpatched.exe && \
        rm ./Quarion.exe ./"place Quarion.exe here.txt"
    else
        pm_message "Error: MD5 checksum of Quarion.exe does not match the expected checksum." 
	exit 1   
    fi
else
    pm_message "Error: Missing files in ./quarion folder OR game has been patched"
fi
