...
# Variables
GAMEDIR="/$directory/ports/damoclesgaze"
BINARY="gamedata/damocles_gaze.app/Contents/MacOS/damocles_gaze"
...
if command -v python3 >/dev/null 2>&1 \
   && [ -f "$GAMEDIR/tools/sugar_fingerprint.py" ]; then
    for candidate in "$PATCHES_DIR"/damocles_gaze*.conf; do
    ...
    case "$BIN_SIZE" in
         4959248)  PATCHES="$PATCHES_DIR/damocles_gaze.conf" ;;
    esac
fi
if [ -z "$PATCHES" ]; then
    pm_message "Unrecognised Damocles Gaze — see BUILDING.md"
    sleep 15
    exit 1
fi
