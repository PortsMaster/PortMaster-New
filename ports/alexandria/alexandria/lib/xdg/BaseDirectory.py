#! /usr/bin/python
#
# Slightly hacked version of the XDG BaseDirectory file from the Debian
# python-xdg to support Windows machines a little better. -EAGC
"""
...
if os.name == 'posix':
	...
	# Note that this "violates" the standard, although it's not clear
	# that XDG was meant to apply to non-POSIX systems. -EAGC
...
xdg_data_home = os.environ.get('XDG_DATA_HOME',
			os.path.join(_home, _local, 'share'))

xdg_data_dirs = [xdg_data_home] + \
	os.environ.get('XDG_DATA_DIRS', '/usr/local/share:/usr/share').split(':')

xdg_config_home = os.environ.get('XDG_CONFIG_HOME',
			os.path.join(_home, _config))

xdg_config_dirs = [xdg_config_home] + \
	os.environ.get('XDG_CONFIG_DIRS', '/etc/xdg').split(':')

xdg_cache_home = os.environ.get('XDG_CACHE_HOME',
			os.path.join(_home, _cache))
