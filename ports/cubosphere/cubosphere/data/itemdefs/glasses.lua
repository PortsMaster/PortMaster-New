...
pickupcolor=COLOR_New(0.4,0.4,0.4,0.3);

function Precache()
  mdl=MDLDEF_Load("glasses");
  PrecacheShadow();
end;
...
function Collide(actorid,itemid)
  ...
  ACTOR_SetVar(actorid,"GoggleTimer",GLOBAL_GetVar("GoggleTime"));
...
function DistRender(item)
   ...
   local picked=ITEM_GetVar(item,"PickedUp");
   if picked==0 then
     RenderShadow(item,height,GLOBAL_GetScale()*0.12);
   else 
     RenderPickup(item);
   end
end
