...
function Precache()
  --print("Precache TDEF");
  diffusemap=TEXTURE_Load("exittile1");

  if GLOBAL_GetVar("ShadersActive")>0 then
     shader=SHADER_Load("distglossbump");
     normalmap=TEXTURE_Load("exittile1_nm");
     glossmap=TEXTURE_Load("exittile1_sm");
  end
end;

function Render(side)
  if TEXDEF_GetLastRenderedType()=="exittile1" then
   if GLOBAL_GetVar("ShadersActive")>0 then
  ...
  LIGHT_SetDiffuse(0,COLOR_New(0.5,0.5,0.5,0));
  ...
  if GLOBAL_GetVar("ShadersActive")>0 then
   ...
   -- MATERIAL_SetSpecularPower(40.0);
    ...
    -- And Ambient etc....
    --lv=VECTOR_New(0,math.sin(GLOBAL_GetTime()),1);
    --LIGHT_SetPosition(0,lv);
