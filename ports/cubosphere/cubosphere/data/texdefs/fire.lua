...
function Precache()
  --print("Precache TDEF");
  diffusemap=TEXTURE_Load("fire");
  if GLOBAL_GetVar("ShadersActive")>0 then
      normalmap=TEXTURE_Load("fire_nm");
  glossmap=TEXTURE_Load("fire_sm");
 
     shader=SHADER_Load("fire");
  end
end;

function Render(side)
  if TEXDEF_GetLastRenderedType()=="fire" then
  if GLOBAL_GetVar("ShadersActive")>0 then
    SHADER_SetVector3("tangent",SIDE_GetTangent(side));
     SHADER_SetFloat("time",(3*GLOBAL_GetTime()+side));
   end;
