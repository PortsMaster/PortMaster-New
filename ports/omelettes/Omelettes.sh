...
# Variables
GAMEDIR="/$directory/ports/omelettes"
...
# Extract file
if [ -f "./assets/Omelettes Eggstended.exe" ]; then
  # Extract its contents in place using 7z
  pm_message "Extracting Omelettes Eggstended.exe ..."
  ./7z x "./assets/Omelettes Eggstended.exe" -o"./assets/" -y
...
if [ -f ./assets/game.droid ]; then
  zip -r -0 ./omelettes.port ./assets/
  rm -Rf ./assets/
fi

# Assign configs and load the game
$GPTOKEYB "gmloadernext.aarch64" -c "omelettes.gptk" &
