...
# Variables
GAMEDIR="/$directory/ports/wratchsden"
BINARY="gamedata/the_wratchs_den.app/Contents/MacOS/the_wratchs_den"

cd "$GAMEDIR"
> "$GAMEDIR/log.txt" && exec > >(tee "$GAMEDIR/log.txt") 2>&1

# First we check for demo version
if [ -f ./gamedata/the_wratchs_den_demo.app/Contents/MacOS/the_wratchs_den_demo ]; then
	mv ./gamedata/the_wratchs_den_demo.app/Contents/MacOS/the_wratchs_den_demo ./gamedata/the_wratchs_den_demo.app/Contents/MacOS/the_wratchs_den
	mv ./gamedata/the_wratchs_den_demo.app/Resources/MacOS/the_wratchs_den_demo.icns  ./gamedata/the_wratchs_den_demo.app/Resources/MacOS/the_wratchs_den.icns 
	mv ./gamedata/the_wratchs_den_demo.app ./gamedata/the_wratchs_den.app 
fi
...
if command -v python3 >/dev/null 2>&1 \
   && [ -f "$GAMEDIR/tools/sugar_fingerprint.py" ]; then
    for candidate in "$PATCHES_DIR"/wratchs-den*.conf; do
    ...
    case "$BIN_SIZE" in
        5399168)  PATCHES="$PATCHES_DIR/wratchs-den.conf" ;;
        4959248)  PATCHES="$PATCHES_DIR/wratchs-den-steam.conf" ;;
    esac
fi
if [ -z "$PATCHES" ]; then
    pm_message "Unrecognised Wratch's Den binary — see BUILDING.md"
    sleep 15
    exit 1
fi
...
$GPTOKEYB "machismo" -c "$GAMEDIR/wratchsden.gptk" &
