...
function osk.show(initialText, onConfirmCallback)
    ...
    currentSelection = 1  -- Janky but prevents double inputs from opening osk again
    visible = true
end
...
function osk.draw()
    ...
    love.graphics.setColor(1, 1, 1)  -- White color for the keys
    ...
    for i, row in ipairs(rows) do
        ...
        for j, key in ipairs(row) do
            ...
            if i == oskSelection[1] and j == oskSelection[2] then
                love.graphics.setColor(0.7, 0.7, 0.7, 0.5)  -- Highlight color
    ...
    love.graphics.setColor(1, 1, 1)  -- Black color for the input text
