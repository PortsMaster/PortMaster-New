...
# Variables
GAMEDIR="/$directory/ports/predetermined"
...
# Extract file
if [ -f "./assets/Predetermined.exe" ]; then
  # Extract its contents in place using 7z
  pm_message "Extracting Predetermined.exe ..."
  ./7z x "./assets/Predetermined.exe" -o"./assets/" -y
...
if [ -f ./assets/game.droid ]; then
  zip -r -0 ./predetermined.port ./assets/
  rm -Rf ./assets/
fi

# Assign configs and load the game
$GPTOKEYB "gmloadernext.aarch64" -c "predetermined.gptk" &
