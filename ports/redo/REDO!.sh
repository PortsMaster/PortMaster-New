...
if [ -n "$(ls ./gamedata/*.ogg 2>/dev/null)" ]; then
    ...
    # Zip the contents of ./redo.apk including the new .ogg files
    zip -r -0 ./redo.apk ./assets/
    echo "Zipped contents to ./redo.apk"
    rm -Rf "$GAMEDIR/assets/"

    # cleanup if extra files were copied in from steam
    rm -Rf "$GAMEDIR/gamedata/REDO!.exe"
fi
